...
#        watchJob(queue, jobnum, simLogFile, 60*60)        turn off for now
        ...
        while time.time() - submittedTime < wallTime:
            interval=10
            print "waiting here for status check every %.1f seconds", interval ###
            time.sleep(interval) # 30
            status_of_check, time_of_check, pendingFlag = returnJobStatus(jobID, submittedTime, pendingFlag)
