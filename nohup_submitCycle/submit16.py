        ...
        while time.time() - submittedTime < wallTime:
            ...
            if status_of_check == "RUNNING":
                ...
                if os.path.exists(simuLogFile):          #TODO need to remove simuLogFile before running this script
                    ...
                    print "waiting %s seconds for %s to be created..." % (wait4SimuLogFile, simuLogFile)
...
def tryJob(job_number, queue, wallTime):
...
#    simLogFile = 'logSimulation0.core480'
    ...
    if queue == 'hsw' :
        ...
        with open(slurmFile,'w') as sf:
            ...
            sf.write("#SBATCH --time=00:10:00\n")      
            sf.write("#SBATCH -C %s\n" % "HSW24")
            sf.write("#SBATCH --qos=bonus") #TODO add \n
            sf.write("#SBATCH --mem=50GB\n")
            sf.write("#SBATCH --output=hsw_mpi.%j.out\n")
            sf.write("#SBATCH --error=hsw_mpi.%j.err")
            sf.write("\n")
        ...
        with open(slurmFile,'w') as sf:
            sf.write("#!/bin/bash\n")
            sf.write("#SBATCH --job-name=Stp0p3_3\n")
            ...
            sf.write("set -e\n")
            sf.write("ulimit -s 500000\n")
            sf.write("srun -n $SLURM_NTASKS nonNewtonianIcoFoamPS_profiling1 -parallel > %s\n" % simuLogFile)
        sf.close()
    else :
        print "no queue named as %s" %slurmFile
...
def RunJobs(pickUpFromJob, nb_jobs, physical_time_interval, endTimeWall, wallTimePerJob):
    queue = sys.argv[1]
    ...
    for i in range(nb_jobs):
        ...
        if ifHittingWall == 0 or ifHittingWall == 1:
            ...
            returnFromCurrentJob = tryJob(i+currentJobNb, queue, wallTimePerJob)
            if returnFromCurrentJob == 0:
                ...
                if returnFromCurrentJob == 1:
                    ...
                    print "Other Error => break" # like exceeded quota => Blocked. Or tryJob wallTimePerJob hit #HINT 1h may be not enough. #TODONew break
                    break
        else :
...
RunJobs(pickUpFromJob=3011,nb_jobs=1000,physical_time_interval=0.0002,endTimeWall=0.8, wallTimePerJob=4*60*60) #TODO read from file per-job so that this is runtime modifiable
