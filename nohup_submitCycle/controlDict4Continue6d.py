...
#
#    if endTime_new == endTimeWall:
#        return False
#    else :
#        return True

def check(endTimeWall):

    lastEndTime, startTime = getTimesFromControlDict('system/controlDict')
    
    if lastEndTime > endTimeWall: # make sure if hitting : lastEndTime = endTimeWall
        endTime_new = endTimeWall
        return 2, startTime, endTime_new
    elif lastEndTime == endTimeWall:
        return 1, startTime, lastEndTime
    else :
        return 0, startTime, lastEndTime

def update(interval, startTime, endTime):
    
    startTime_new = endTime
    endTime_new   = endTime + interval
