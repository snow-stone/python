...
def plotCaseWithSlices_0p8_MI_Alex(ax_cases, path2Data, dataDir, positionList, aliasDict, markerDict, colorDict, cut, ifPlotInter):
    meanOfMEAN = np.zeros(len(positionList))
    meanOfRMS = np.zeros(len(positionList))
...
def plotCaseWithSlices_Dai_MI_Alex(ax_cases, path2Data, dataDir, positionList, aliasDict, markerDict, colorDict, cut, ifPlotInter):
    meanOfMEAN = np.zeros(len(positionList))
    meanOfRMS = np.zeros(len(positionList))
...
#        plotCaseWithSlices_local_rms(axes[1], path2Data, caseDir, positionList, aliasDict, markerDict, colorDict, cut=0.7, ifPlotInter=False)
    ...
    axes[1].set_ylabel(r"$<T^{\prime 2}>_{slice}^{1/2}$")
...
#        plotCaseWithSlices_Dai_MI(axes[2], path2Data, caseDir, positionList, aliasDict, markerDict, colorDict, cut=0.7, ifPlotInter=False)
        if caseDir == "BirdCarreau/inlet0p5_impinging" or caseDir == "Newtonian/Re4000_impinging":
            plotCaseWithSlices_0p8_MI_Alex(axes[2], path2Data, caseDir, positionList, aliasDict, markerDict, colorDict, cut=0.7, ifPlotInter=False)    
        else :
            plotCaseWithSlices_Dai_MI_Alex(axes[2], path2Data, caseDir, positionList, aliasDict, markerDict, colorDict, cut=0.7, ifPlotInter=False)
        
    axes[2].set_xlabel(r"$x/D$")
    axes[2].set_ylabel(r"$<T^{\prime 2}>_{slice}^{1/2}/<T>_{slice}$")
...
#    fig.savefig(path2Data+"/"+saveFigDir+'/mixing.png', bbox_inches='tight')
    fig.savefig(path2Data+"/"+saveFigDir+'/mixing_Alex.png', bbox_inches='tight')
