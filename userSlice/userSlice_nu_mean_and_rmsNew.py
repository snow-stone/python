...
def nu_mean_Figure_ref_local(caseList, path2Data, saveFigDir):
    ...
    def plotSlice_local(ax, sliceNumber, path2Data, dataDir, cut=0.5, ifPlotInter=True):
        data = np.genfromtxt(path2Data+"/"+dataDir+"/"+"userDefinedLog/nu_mean_slice"+str(sliceNumber)+"_mean_rmsNew")
...
#    ax_principle.set_xlim(0,40)
    axes[0].legend(bbox_to_anchor=(0.7, 0.97), ncol=3, shadow=True, fontsize=20, handlelength=2.5)
...
#    fig.savefig(path2Data+"/"+saveFigDir+'/mixing.png', bbox_inches='tight')
    axes[0].text(-0.1,2.2,'(a)',size=30, transform=ax.transAxes)
    axes[1].text(-0.1,1,'(b)',size=30, transform=ax.transAxes)
    #axes[2].text(-0.1,1,'(c)',size=20, transform=ax.transAxes)
    fig.savefig(path2Data+"/"+saveFigDir+'/nu_mixing_ref_local_1New.png', bbox_inches='tight', fontsize=20)
