...
#        ax1.plot(dataBase2Plot['rByD'],dataBase2Plot['chunkedMean'][i]/Ux_bulk_Dai,label=str(i),linewidth=4)
    #ax1.plot(dataBase2Plot['rByD'],dataBase2Plot['chunkedMean'][1]/Ux_bulk_Dai,label=alias_dict[parameterFileBasename],linewidth=4)
    ax1.plot(dataBase2Plot['rByD'],rdb.smoothFunction.movingAvg(dataBase2Plot['chunkedMean'][1]/Ux_bulk_Dai,3),label=alias_dict[parameterFileBasename],linewidth=4)
...
#        ax2.plot(dataBase2Plot['rByD'],dataBase2Plot['chunkedStd'][i]/Ux_bulk_Dai,label=str(i),linewidth=4)
    ax2.plot(dataBase2Plot['rByD'],dataBase2Plot['chunkedStd'][1]/Ux_bulk_Dai,label=alias_dict[parameterFileBasename],linewidth=4)
...
#   plot settings    
    ...
    if ifLocalControl == "ControlFig_usingParameters":
        ...
        print "====================================="
        ax1.legend(bbox_to_anchor=simu_parameters['plot']['legendPosition2a'], ncol=2, fancybox=True, shadow=True)
    else:
        print "====================================="
        print "For fig1 :"
        print "In file " + os.path.basename(__file__)
    ...
    if ifLocalControl == "ControlFig_usingParameters":
        ...
        print "applying ControlFig_usingParameters pre-described in file " + os.path.basename(simu_module.__file__)
        print "====================================="
        ax2.legend(bbox_to_anchor=simu_parameters['plot']['legendPosition2b'], ncol=2, fancybox=True, shadow=True)
    else:
        print "====================================="
        print "For fig2 :"
        print "In file " + os.path.basename(__file__)
