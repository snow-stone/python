...
#   get data    
    simu_parameters1['sampling']['dataShape']=simu_parameters1['sampling']['TdataShape3']
    fileListSimu = tsR.pre_check(simu_parameters1,"Dai_lines_typeFace_cell_T-3")
    dataBase2Plot1 = tsR.process(simu_parameters1,validDataList=fileListSimu,colonNb=1)  

    simu_parameters2['sampling']['dataShape']=simu_parameters2['sampling']['TdataShape3']
    fileListSimu = tsR.pre_check(simu_parameters2,"Dai_lines_typeFace_cell_T-3")
...
#   No-dimnesionize and plot
    ...
    y0 = y0/2.0
    ax1.plot(dataBase2Plot1['rByD'],y0,label=alias_dict[parameterFileBasename1],linewidth=4)

    y0a = dataBase2Plot2['chunkedMean'][4]+dataBase2Plot2['chunkedMean'][5]
    y0a = y0a/2.0
    ax1.plot(dataBase2Plot2['rByD'],y0a,label=alias_dict[parameterFileBasename2],linewidth=4)
    ...
    y1 = dataBase2Plot1['chunkedStd'][3]+dataBase2Plot1['chunkedStd'][4]
    y1 = y1/2.0
    y1 = rdb.smoothFunction.movingAvg(y1,3)
    ax2.plot(dataBase2Plot1['rByD'],y1,label=alias_dict[parameterFileBasename1],linewidth=4)

    y1a = dataBase2Plot2['chunkedStd'][3]+dataBase2Plot2['chunkedStd'][4]
    y1a = y1a/2.0
    y1a = rdb.smoothFunction.movingAvg(y1a,3)
    ax2.plot(dataBase2Plot2['rByD'],y1a,label=alias_dict[parameterFileBasename2],linewidth=4)
...
#   plot settings    
    ...
    if ifLocalControl == "ControlFig_usingParameters":
        ...
        print "====================================="
        ax1.legend(bbox_to_anchor=simu_parameters1['plot']['legendPosition2a'], ncol=2, fancybox=True, shadow=True)
    else:
        print "====================================="
        print "For fig1 :"
        print "In file " + os.path.basename(__file__)
    ...
    if ifLocalControl == "ControlFig_usingParameters":
        ...
        print "applying ControlFig_usingParameters pre-described in file " + os.path.basename(simu_module1.__file__)
        print "====================================="
        ax2.legend(bbox_to_anchor=simu_parameters1['plot']['legendPosition2b'], ncol=2, fancybox=True, shadow=True)
    else:
        print "====================================="
        print "For fig2 :"
        print "In file " + os.path.basename(__file__)
