...
#            ]
    ...
    for i, sample in enumerate(samples):
        ...
        fig.savefig("../PICTURE_history/"+arrayName+"/x_Eq_"+str(positionList[sample]/8.0).replace('.','p')+"D.png", bbox_inches='tight',dpi=300) # bbox_inches = 'tight' is neccessary
#==============================================================================
#   RMS 
#==============================================================================    
    fig_rms = plt.figure()
...
#    ax2 = fig_rms.add_subplot(2,1,2)
    for i, case in enumerate(debit_min):
        ax1.plot(positionList,std_min[:,i],label=debit_min[i],linestyle=linestyle_min[i])
    ax1.legend(bbox_to_anchor=(1., 1), ncol=1, shadow=True)
    ax1.set_xlabel(r"$x/D$")
    ax1.set_ylabel(latexRMSName)
#    ax1.set_ylim(0,0.4)
    
    fig_rms.savefig('../PICTURE_history/'+arrayName+'/RMS_xByD_oneFig.png', bbox_inches='tight',  dpi=300)
#==============================================================================
#   Mean
#==============================================================================
    fig_rms = plt.figure()
...
#    ax2 = fig_rms.add_subplot(2,1,2)
    for i, case in enumerate(debit_min):
        ax1.plot(positionList,mean_min[:,i],label=debit_min[i],linestyle=linestyle_min[i])
    ax1.legend(bbox_to_anchor=(1., 1), ncol=1, shadow=True)
    ax1.set_xlabel(r"$x/D$")
    ax1.set_ylabel(latexMEANName)
    
    fig_rms.savefig('../PICTURE_history/'+arrayName+'/Mean_xByD_oneFig.png', bbox_inches='tight', dpi=300)
