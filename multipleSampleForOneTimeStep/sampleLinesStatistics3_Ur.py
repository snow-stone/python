...
def sampleLinesStatistics1d(validDataList,uTau,ifPlotAllTimes=False):
...
#    print "average velocity : ", uTau * integral_2D / (np.pi * 1.0**2)
    fig,ax = plt.subplots()
    ax.plot(x,mean[:,1],label='mean',color='red',linewidth=2)
    if ifPlotAllTimes:
        for i in range(0, sampleSize, 20):
            ax.plot(x,data[i][:,1],label=str(i))
    ...
    ax2.plot(rPlus,std[:,1][::-1],label='simu',color='red',marker='^')
...
def main():
    ...
    ax, ax2 = sampleLinesStatistics1d(validDataList=l,uTau=0.0473,ifPlotAllTimes=True)
    sizeLabel = 15
    
    ax.set_xlabel(r'$r$'+' from center to wall',fontsize=sizeLabel)
    ax.set_ylabel(r'$<U_r>^+$',fontsize=sizeLabel)
    
    uTau=0.0473
    R=0.004
    y_, UzPlus = db.analytic_Uz_meanProfile(uTau,100)
    y_=y_/R
    y_=-y_+1
#    ax.plot(y_[::-1],UzPlus[::-1],label='mean_objectif',color='blue',linewidth=2)
    ax.set_xlim(0,1)
    ax.legend(bbox_to_anchor=(1.5, 1.2), ncol=1, fancybox=True, shadow=True)
    
    x2, y2 = db.Niewstadt1995_pipe_UrRMS()
    ax2.plot(x2, y2, label='data Niewstadt1995_pipe', marker='o')
    ax2.set_xlabel(r'$r^+$'+' from wall to center',fontsize=sizeLabel)
    ax2.set_ylabel(r'$rmsU_r^+$',fontsize=sizeLabel)
    ax2.set_xscale('log')
    
    deg=4
    x_PolyFit, y_PolyFit = db.dataFitting_Niewstadt1995_pipe_UrRMS(deg=deg,samplingSize=60)
