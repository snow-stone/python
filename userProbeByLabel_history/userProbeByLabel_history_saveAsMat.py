...
def userProbeByLabel_forcing(ax, caseName, path2Data, fieldName, sample, positions, color, cut=0.0, signal={}):
    ...
    signal[caseName]= probeData[:cutSliceIndex,sample]

    return signal, std, mean

def userProbeByLabel(ax, caseName, path2Data, fieldName, sample, positions, color, cut=0.5, signal={}):
    RelativeDataFile = "./"+"userDefinedLog/history_labelGroup_"+fieldName
    rawData = np.genfromtxt(path2Data+"/"+caseName+'/'+RelativeDataFile)
    time = rawData[:,0]
    probeData = rawData[:,1:]
...
#    rc('font',**{'family':'sans-serif','sans-serif':['Helvetica']})
    ...
    import scipy.io as io # saving data (must be a dictionary) to *.mat format
    signal={}
    ...
    for k, position in enumerate(positionSubSet):
        ...
        #        spatial_mean_rms(arrayName, cases, positionList, mean, std, linestyleList, markerList)
                ...
                if case == "BirdCarreau/inlet_0p3-a_0p5-setT_St_1" or case == "BirdCarreau/inlet_0p3-a_0p5-setT_St_5":
                    signal, std[i,j], mean[i,j] = userProbeByLabel_forcing(axses_case[j], case, path2Data,  fieldName, p, allProbePosition, colorDict[fieldName], cutDict[case], signal)
                else:
                    signal, std[i,j], mean[i,j] = userProbeByLabel(axses_case[j], case, path2Data, fieldName, p, allProbePosition, colorDict[fieldName], cutDict[case], signal)
        ...
        print signal["BirdCarreau/inlet0p5_impinging"]
        print "\n"
        print signal["Newtonian/Re4000_impinging"]
        io.savemat("signal_U_y_2D",signal)
